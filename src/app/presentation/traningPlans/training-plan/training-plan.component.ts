...
export class TrainingPlanComponent implements OnInit {
  ...
  hoveredDay: { weekNumber: number | null, dayNumber: number | null } = { weekNumber: null, dayNumber: null };
  ...
  constructor(private trainingPlanService: TrainingPlansService, private router: ActivatedRoute) {}
  ...
  getTrainingPlanById(planId: string | number): void {
    ...
    this.trainingPlanService.getTrainingPlanById(Number(id)).subscribe({
      next: (response) => {
        ...
        if (this.trainingPlan.weekPlans && this.trainingPlan.weekPlans.length > 0) {
  ...
  createWeekPlan(weekPlan: CreateWeekPlanDto): void {
    ...
    this.trainingPlanService.createWeekPlan(weekPlan).subscribe({
      next: (newWeekPlan) => {
        ...
        const currentWeekNumber = parseInt(this.newWeekPlan.weekName.split(' ')[1]);
        console.log('Target week number:', currentWeekNumber);
        
        // Find the week in our array
        const weekIndex = this.weeks.findIndex(w => w.weekNumber === currentWeekNumber);
  ...
  createDayPlan(dayPlan: CreateDailyPlanDto): void {
    this.trainingPlanService.createDailyPlan(dayPlan).subscribe({
      next: (newDayPlan) => {
        ...
        const week = this.weeks.find(w => w.weekPlanId && w.weekPlanId.toString() === dayPlan.weekPlanId.toString());
        if (week) {
          ...
          console.log(`Added day plan for week ${week.weekNumber}, day ${dayPlan.dayNumber}`);
  ...
  generateMonths(): void {
    if (this.trainingPlan && this.trainingPlan.durationMonths) {
      this.months = Array.from({ length: this.trainingPlan.durationMonths }, (_, i) => i + 1);
      console.log(`Generated ${this.months.length} months for the training plan`);
    }
  }
  ...
  mapWeekPlansToWeeks(weekPlans: WeekPlanDto[]): void {
    ...
    for (const weekPlan of weekPlans) {
      ...
      if (weekPlan.title) {
        ...
        if (titleMatch && titleMatch[1]) {
          ...
          console.log(`Extracted week number ${weekNumber} from title: ${weekPlan.title}`);
          
          // Find the week with this number
          const week = this.weeks.find(w => w.weekNumber === weekNumber);
          if (week) {
            console.log(`Successfully mapped week plan to week ${weekNumber} by title`);
      ...
      if (!matched && (weekPlan as any).startDate) {
        ...
        if (matchingWeek) {
          console.log(`Successfully mapped week plan to week ${matchingWeek.weekNumber} by date range`);
          ...
          console.warn(`No week found containing date: ${this.formatDate(weekPlanDate)}`);
        }
      }
      ...
      if (!matched && weekPlan.id !== undefined && weekPlan.id !== null) {
        const possibleWeek = this.weeks.find(w => w.weekNumber === Number(weekPlan.id));
        if (possibleWeek) {
    ...
    console.log(`Successfully mapped ${mappedWeeks.length} out of ${weekPlans.length} week plans`);
    mappedWeeks.forEach(week => {
      console.log(`Week ${week.weekNumber} (${this.formatDate(week.startDate)} - ${this.formatDate(week.endDate)}) has plan: ${week.weekPlanId}`);
  ...
  mapWorkoutDaysToDailyPlans(week: Week, weekPlan: WeekPlanDto): void {
    ...
    if (weekPlan && (weekPlan as any).workoutDays && Array.isArray((weekPlan as any).workoutDays)) {
      console.log(`Found ${(weekPlan as any).workoutDays.length} workout days for week plan ${weekPlan.id}`);
      ...
      (weekPlan as any).workoutDays.forEach((workoutDay: any) => {
        if (workoutDay && workoutDay.dayDate) {
          try {
            ...
            console.log(`Processing workout day for date: ${this.formatDate(dayDate)}`);
            ...
            if (this.isDateInRange(dayDate, week.startDate, week.endDate)) {
              ...
              week.dailyPlans[dayOfWeek.toString()] = {
                ...
                dayNumber: dayOfWeek,
                dayDate: dayDate,
                exercise: workoutDay.exercise || workoutDay.exerciseName || 'Training'
              };
              
              console.log(`Mapped workout day to day ${dayOfWeek} in week ${week.weekNumber}`);
            } else {
              console.warn(`Workout day date ${this.formatDate(dayDate)} is outside week range ${this.formatDate(week.startDate)} - ${this.formatDate(week.endDate)}`);
              
              // Try to find the correct week for this workout day
              const correctWeek = this.weeks.find(w => this.isDateInRange(dayDate, w.startDate, w.endDate));
              if (correctWeek) {
                // Determine the day of the week (1-7, where 1 is Monday)
                const dayOfWeek = this.getDayOfWeekNumber(dayDate);
                ...
                console.log(`Remapped workout day to correct week ${correctWeek.weekNumber}, day ${dayOfWeek}`);
              } else {
                console.warn(`Could not find a suitable week for workout day on ${this.formatDate(dayDate)}`);
              }
            }
          } catch (error) {
            console.error(`Error parsing workout day date: ${workoutDay.dayDate}`, error);
          }
          ...
          if (!isNaN(dayNum) && dayNum >= 1 && dayNum <= 7) {
            ...
            dayDate.setDate(dayDate.getDate() + (dayNum - this.getDayOfWeekNumber(week.startDate)));
            
            console.log(`Using day number ${dayNum} to calculate date: ${this.formatDate(dayDate)}`);
            ...
            console.log(`Mapped workout day to day ${dayNum} in week ${week.weekNumber}`);
          }
        }
      });
    } else {
      console.log(`No workout days found for week plan ${weekPlan?.id || 'unknown'}`);
    }
  }
  ...
  loadDailyPlansForWeek(week: Week): void {
    ...
    console.log(`Would load daily plans for week ${week.weekNumber} with week plan ID ${week.weekPlanId}`);
  ...
  openAddWeekPopup(week: Week): void {
    ...
    console.log(`Week dates: ${this.formatDate(week.startDate)} - ${this.formatDate(week.endDate)}`);
    ...
    console.log(`Setting start date to day after week start: ${this.formatDate(dayAfterStart)}`);
  ...
  openAddDayPopup(week: Week, dayNumber: number): void {
    if (!week.hasWeekPlan || (week.dailyPlans && week.dailyPlans[dayNumber.toString()])) return; // Don't open if no week plan or day already has a plan
  ...
  isDayHovered(weekNumber: number, dayNumber: number): boolean {
    return this.hoveredDay.weekNumber === weekNumber && this.hoveredDay.dayNumber === dayNumber;
  }
  ...
  processNewDayPlan(dayPlan: any): void {
    ...
    if (!dayPlan || !dayPlan.weekPlanId) {
      console.error('Cannot process day plan: invalid data or missing weekPlanId');
      return;
    }
    
    // Find the corresponding week
    const week = this.weeks.find(w => w.weekPlanId && w.weekPlanId === dayPlan.weekPlanId.toString());
    if (week) {
      // Initialize dailyPlans if it doesn't exist
      if (!week.dailyPlans) {
        week.dailyPlans = {};
      ...
      // Store the daily plan by day number
      const dayNumber = typeof dayPlan.dayNumber === 'string' ? parseInt(dayPlan.dayNumber, 10) : dayPlan.dayNumber;
      if (week.dailyPlans) {
        week.dailyPlans[dayNumber.toString()] = dayPlan;
        console.log(`Added day plan for week ${week.weekNumber}, day ${dayNumber}`);
        console.log('Updated week:', week);
      }
    }
  ...
  setDayPlanForDay(weekIndex: number, day: Day, dailyPlanId: string | undefined): void {
