...
export class TrainingPlanComponent implements OnInit {
  ...
  submitWeekPlan(): void {
    // Validate inputs
    if (!this.newWeekPlan?.weekName) {
      console.error('Invalid weekName:', this.newWeekPlan?.weekName);
      return;
    }

    // Extract the numeric part from weekName (e.g., "Week 2" -> "2")
    const weekNameMatch = this.newWeekPlan.weekName.match(/\d+/);
    if (!weekNameMatch) {
      console.error(
        'No valid number found in weekName:',
        this.newWeekPlan.weekName
      );
      return;
    }

    const weekNumber = parseInt(weekNameMatch[0], 10);
    if (isNaN(weekNumber)) {
      console.error(
        'Parsed weekNumber is not a valid number:',
        weekNameMatch[0]
      );
      return;
    }
    ...
    if (weekIndex === -1) {
      console.error(
        `No week found with weekNumber ${weekNumber}. Available weeks:`,
        this.weeks
      );
      return;
    }
    ...
    this.trainingPlanService.createWeekPlan(this.newWeekPlan).subscribe({
      next: (newWeekPlan) => {
        ...
        if (newWeekPlan.id) {
          ...
          console.warn('No ID returned for new week plan:', newWeekPlan);
        }

        // Initialize daily plans if needed
        this.weeks[weekIndex].dailyPlans =
          this.weeks[weekIndex].dailyPlans || {};
      ...
      error: (error) => {
        console.error('Error creating week plan:', error.message, error);
      },
    });
  }
  submitDayPlan(): void {
