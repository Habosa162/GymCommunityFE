...
export class MyClientsComponent implements OnInit {
  ...
  loadClients(): void {
    this.coachClientsService.getClientsByCoachId(this.currentPage, this.pageSize).subscribe({
      next: (response: any) => {
  ...
  sortClients(criteria: string): void {
    switch (criteria) {
      ...
      case 'age':
        this.filteredClients.sort((a, b) => 
          this.calculateAge(a.client?.birthDate) - this.calculateAge(b.client?.birthDate));
        break;
    }
  }
